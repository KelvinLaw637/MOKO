...
- (void)dataCache:(SDDataCache *)dataCache didFindData:(NSData *)aData forKey:(NSString *)key userInfo:(NSDictionary *)info
{
	...
	if ([delegate respondsToSelector:@selector(webDataManager:didFinishWithData:)])
	{
		[delegate performSelector:@selector(webDataManager:didFinishWithData:) withObject:self withObject:aData];
	}
}
